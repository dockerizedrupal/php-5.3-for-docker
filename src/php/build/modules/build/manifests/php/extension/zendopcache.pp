class build::php::extension::zendopcache {
  require build::php
  ...
  bash_exec { 'cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.3.29':
    timeout => 0,
  ...
  bash_exec { 'cd /tmp/zendopcache-7.0.3 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.3.29']
  }
