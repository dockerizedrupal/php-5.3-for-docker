class php::extension::redis {
  ...
  exec { '/bin/su - root -c "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-redis-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/redis-2.2.5 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/redis-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-redis-igbinary"']
  }
