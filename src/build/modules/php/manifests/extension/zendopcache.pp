class php::extension::zendopcache {
  ...
  exec { '/bin/su - root -c "cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/zendopcache-7.0.3 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/zendopcache-7.0.3 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29"']
  }
