class php::extension::memcached {
  ...
  exec { '/bin/su - root -c "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/memcached-2.2.0 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary"']
  }
