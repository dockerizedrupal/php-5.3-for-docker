class php::extension::memcached {
  ...
  exec { '/bin/su - root -mc "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary"':
    timeout => 0,
    require => Exec['phpize-5.3.29 memcached']
  }

  exec { '/bin/su - root -mc "cd /tmp/memcached-2.2.0 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary"']
  }

  exec { '/bin/su - root -mc "cd /tmp/memcached-2.2.0 && make install"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/memcached-2.2.0 && make"']
  }
}
