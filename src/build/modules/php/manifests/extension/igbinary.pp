class php::extension::igbinary {
  ...
  exec { '/bin/su - root -c "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/igbinary-1.2.1 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29 --enable-igbinary"']
  }
