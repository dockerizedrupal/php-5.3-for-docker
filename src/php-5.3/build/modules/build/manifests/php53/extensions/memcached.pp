class build::php53::extensions::memcached {
  require build::php53
  require build::php53::extensions::igbinary
  ...
  bash_exec { 'cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary':
    timeout => 0,
  ...
  bash_exec { 'cd /tmp/memcached-2.2.0 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.3.29 --enable-memcached-igbinary']
  }
