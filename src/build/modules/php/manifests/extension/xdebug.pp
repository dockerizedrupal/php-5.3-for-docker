class php::extension::xdebug {
  ...
  exec { '/bin/su - root -c "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29"':
    timeout => 0,
  ...
  exec { '/bin/su - root -c "cd /tmp/xdebug-2.2.6 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -c "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.3.29"']
  }
